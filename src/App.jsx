...
const BillingCard = ({ label, count, hours, onEdit, onExport }) => (
  // Gradient: #0F2C59 (Navy) -> #DAC0A3 (Taupe/Gold accent)
  <div className="billing-card" style={{ background: 'linear-gradient(135deg, #0F2C59 0%, #2a4d82 100%)', borderRadius: '16px', padding: '24px', color: '#F8F0E5', boxShadow: '0 10px 15px -3px rgba(15, 44, 89, 0.3)', position: 'relative', border: '1px solid #DAC0A3' }}>
    <div style={{ position: 'absolute', top: '20px', right: '20px', display: 'flex', gap: '8px' }}>
      <button onClick={onExport} style={{ background: 'rgba(248, 240, 229, 0.1)', border: 'none', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#EADBC8' }} title="Export Excel">
        <Download size={16} />
      </button>
      <button onClick={onEdit} style={{ background: 'rgba(248, 240, 229, 0.1)', border: 'none', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#EADBC8' }} title="Edit Billing Cycle">
        <Settings size={16} />
      </button>
    </div>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '16px' }}>
      <div><p style={{ fontSize: '13px', fontWeight: '600', opacity: 0.8, textTransform: 'uppercase', letterSpacing: '0.5px', color: '#EADBC8' }}>Current Billing Cycle</p><p style={{ fontSize: '14px', fontWeight: 'bold', marginTop: '4px', color: '#F8F0E5' }}>{label}</p></div>
      <CalendarDays size={24} style={{ opacity: 0.8, marginRight: '80px', color: '#EADBC8' }} />
    </div>
    <div className="billing-stats-grid">
      <div><h3 style={{ fontSize: '32px', fontWeight: '800', lineHeight: '1' }}>{count}</h3><p style={{ fontSize: '13px', opacity: 0.8, marginTop: '4px', color: '#EADBC8' }}>Files Completed</p></div>
      <div className="billing-separator"><h3 style={{ fontSize: '32px', fontWeight: '800', lineHeight: '1' }}>{hours}</h3><p style={{ fontSize: '13px', opacity: 0.8, marginTop: '4px', color: '#EADBC8' }}>Audio Hours</p></div>
    </div>
  </div>
);
...
const StatCard = ({ title, value, icon: Icon, color }) => (
  ...
  <div className="stat-card" style={{ borderLeft: `4px solid ${color}`, backgroundColor: 'white' }}>
    <div className="stat-content"><p className="stat-title" style={{color: '#0F2C59', opacity: 0.7}}>{title}</p><h3 className="stat-value" style={{color: '#0F2C59'}}>{value}</h3></div>
...
const StatusBadge = ({ status }) => {
  ...
  // Navy (#0F2C59), Beige (#EADBC8), Taupe (#DAC0A3)
  const c = { 
    'Completed': {bg:'#EADBC8', t:'#0F2C59', b:'#DAC0A3'}, 
    'In Progress': {bg:'#F8F0E5', t:'#0F2C59', b:'#0F2C59'}, 
    'Pending QA': {bg:'#DAC0A3', t:'#0F2C59', b:'#0F2C59'} 
...
export default function App() {
  ...
  // Palette:
  // #0F2C59 (Navy) -> Sidebar, Primary Buttons, Headings
  // #F8F0E5 (Off-White) -> Main Background
  // #DAC0A3 (Taupe) -> Active States, Borders
  // #EADBC8 (Almond) -> Secondary Accents

  const styles = {
    container: { fontFamily: 'Inter, sans-serif', backgroundColor: '#F8F0E5', minHeight: '100vh', display: 'flex', flexDirection: 'column' },
    
    // SIDEBAR: Navy Background
    sidebar: { width: '250px', backgroundColor: '#0F2C59', borderRight: '1px solid #DAC0A3', display: 'flex', flexDirection: 'column', position: 'fixed', height: '100%', zIndex: 50, transition: 'transform 0.3s ease', transform: isMobile && !showMobileMenu ? 'translateX(-100%)' : 'translateX(0)' },
    
    main: { flex: 1, marginLeft: isMobile ? '0' : '250px', padding: isMobile ? '1rem' : '2rem', overflowY: 'auto' },
    
    // NAV BUTTON: Off-White Text when inactive, Taupe Pill when active
    navBtn: { display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 20px', width: '100%', background: 'transparent', border: 'none', color: '#EADBC8', cursor: 'pointer', fontSize: '14px', fontWeight: '500', transition: 'all 0.2s', margin: '4px 0', borderRadius: '0 20px 20px 0', opacity: 0.8 },
    navBtnActive: { backgroundColor: '#DAC0A3', color: '#0F2C59', fontWeight: '700', opacity: 1 },
    
    // INPUTS: White on Light Background
    input: { width: '100%', padding: '10px 14px', borderRadius: '8px', border: '1px solid #DAC0A3', fontSize: '14px', outline: 'none', backgroundColor: '#ffffff', boxSizing:'border-box', color: '#0F2C59' },
    label: { display: 'block', marginBottom: '6px', fontSize: '14px', fontWeight: '600', color: '#0F2C59' },
    
    // TABLES & CARDS: White Background
    table: { width: '100%', borderCollapse: 'collapse', backgroundColor: '#ffffff', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 4px 6px -1px rgba(15, 44, 89, 0.1)' },
    th: { backgroundColor: '#EADBC8', color: '#0F2C59', padding: '12px 16px', textAlign: 'left', fontSize: '11px', fontWeight: '800', textTransform: 'uppercase', borderBottom: '1px solid #DAC0A3' },
    thClickable: { cursor: 'pointer', userSelect: 'none', display:'flex', alignItems:'center', gap:'6px' },
    td: { padding: '14px 16px', borderBottom: '1px solid #F8F0E5', fontSize: '14px', color: '#0F2C59' },
    tdWrapper: { width: '100%', height: '100%', whiteSpace: 'nowrap', overflowX: 'auto', overflowY: 'hidden', display: 'block' },
    radioLabel: { display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', cursor: 'pointer', padding: '10px', borderRadius: '8px', border: '1px solid #DAC0A3', backgroundColor: '#ffffff', color: '#0F2C59' },
    radioActive: { backgroundColor: '#EADBC8', borderColor: '#0F2C59', color: '#0F2C59', fontWeight: '700' },
    primaryBtn: { backgroundColor: '#0F2C59', color: '#F8F0E5', padding: '8px 16px', borderRadius: '8px', border: 'none', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', boxShadow: '0 2px 4px rgba(15, 44, 89, 0.2)' },
    
    timerDisplay: { fontSize: '48px', fontWeight: 'bold', fontFamily: 'monospace', color: '#0F2C59', textAlign: 'center', margin: '20px 0' },
    ...
    startBtn: { backgroundColor: '#0F2C59', color: '#F8F0E5' },
    pauseBtn: { backgroundColor: '#DAC0A3', color: '#0F2C59' },
    stopBtn: { backgroundColor: '#ef4444', color: 'white' },
    stageOption: { display: 'flex', flexDirection: 'column', padding: '15px', borderRadius: '8px', border: '2px solid #DAC0A3', flex: 1, textAlign:'center' },
    stageActive: { borderColor: '#0F2C59', backgroundColor: '#EADBC8' },
    stageTitle: { fontWeight: 'bold', marginBottom: '4px', color: '#0F2C59' },
  ...
  return (
    <div style={styles.container}>
      <style>{` 
        ...
        .billing-separator { border-left: 1px solid rgba(248, 240, 229, 0.3); padding-left: 20px; }
        @media (max-width: 768px) {
            .billing-stats-grid { grid-template-columns: 1fr; gap: 10px; }
            .billing-separator { border-left: none; padding-left: 0; padding-top: 10px; border-top: 1px solid rgba(248, 240, 229, 0.3); }
      ...
      {isMobile && (
        <div style={{ padding: '16px', background: '#0F2C59', borderBottom: '1px solid #DAC0A3', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 60 }}>
            <div style={{display:'flex', alignItems:'center', gap:'12px'}}>
                <button onClick={() => setShowMobileMenu(!showMobileMenu)} style={{border:'none', background:'none'}}><Menu size={24} color="#F8F0E5"/></button>
                <span style={{fontWeight:'bold', color:'#F8F0E5'}}>TrackScribe</span>
            </div>
            <button onClick={openNewEntry} style={{backgroundColor:'#DAC0A3', color:'#0F2C59', border:'none', padding:'6px 12px', borderRadius:'6px', fontSize:'12px'}}>+ Add</button>
        </div>
      )}

      <aside style={styles.sidebar}>
        <div style={{ padding: '24px', borderBottom: '1px solid rgba(248, 240, 229, 0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ fontSize: '18px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '10px', margin: 0 }}>
            <div style={{ background: '#F8F0E5', width: '28px', height: '28px', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#0F2C59', fontWeight: '900' }}>T</div>
            <span style={{color: '#F8F0E5'}}>TrackScribe</span>
          </h2>
          {isMobile && <button onClick={() => setShowMobileMenu(false)} style={{background:'none', border:'none', color:'#F8F0E5'}}><X size={20}/></button>}
        </div>
      ...
      {isMobile && showMobileMenu && (
        <div onClick={() => setShowMobileMenu(false)} style={{position:'fixed', inset:0, background:'rgba(15, 44, 89, 0.8)', zIndex:40}} />
      )}

      <main style={styles.main}>
        {loading ? <div style={{display:'flex', justifyContent:'center', marginTop:'50px'}}><Loader2 className="animate-spin" color="#0F2C59" /></div> : (
          <>
            {view === 'dashboard' && (
              <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
                {!isMobile && (
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                    <h2 style={{ fontSize: '24px', fontWeight: 'bold', color: '#0F2C59', margin: 0 }}>Dashboard</h2>
                ...
                <div className="dashboard-grid">
                    <StatCard title="Total Lifetime Files" value={jobs.filter(j => j.status === 'Completed').length} icon={CheckCircle2} color="#0F2C59" />
                    <StatCard title="Pending Review" value={jobs.filter(j => j.status === 'Pending QA').length} icon={AlertCircle} color="#DAC0A3" />
                </div>

                <div style={{ background: '#ffffff', padding: '24px', borderRadius: '16px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}><h3 style={{ fontWeight: 'bold', marginBottom: '20px', fontSize: '14px', textTransform:'uppercase', color:'#0F2C59' }}>Weekly Output (Minutes)</h3><div style={{ height: '250px' }}><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#DAC0A3" /><XAxis dataKey="date" axisLine={false} tickLine={false} tickFormatter={(str) => new Date(str).toLocaleDateString(undefined, {weekday: 'short'})} /><YAxis axisLine={false} tickLine={false} /><Tooltip cursor={{fill: 'transparent'}} /><Bar dataKey="minutes" fill="#0F2C59" radius={[4, 4, 4, 4]} barSize={32} /></BarChart></ResponsiveContainer></div></div>
              </div>
            )}

            {showBillingModal && (
              <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(15, 44, 89, 0.8)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
                <div style={{ backgroundColor: '#F8F0E5', borderRadius: '16px', width: '100%', maxWidth: '350px', padding: '24px' }}>
                  <h3 style={{ margin: '0 0 16px 0', fontSize: '16px', fontWeight: 'bold', color: '#0F2C59' }}>Billing Settings</h3>
                  
                  <div style={{marginBottom:'12px'}}>
                      <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#0F2C59', marginBottom: '6px', textTransform: 'uppercase' }}>Start Date</label>
                      <input 
                        type="date" 
                  ...
                  <div style={{marginBottom:'16px'}}>
                      <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#0F2C59', marginBottom: '6px', textTransform: 'uppercase' }}>End Date</label>
                      <input 
                        type="date" 
                  ...
                  }} style={{ width: '100%', marginTop: '10px', padding: '10px', background: '#0F2C59', color: '#F8F0E5', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>Save Changes</button>
                  <button onClick={() => setShowBillingModal(false)} style={{ width: '100%', marginTop: '10px', padding: '10px', background: 'transparent', color: '#0F2C59', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>Cancel</button>
                </div>
              </div>
            )}
            ...
            {view === 'timer' && (
              <div style={{ maxWidth: '600px', margin: '0 auto' }}>
                <h2 style={{ fontSize: '24px', fontWeight: 'bold', color: '#0F2C59', marginBottom: '24px' }}>TAT Timer</h2>
                ...
                <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '16px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
                  ...
                  <div style={{display:'flex', gap:'10px', marginBottom:'24px'}}>
                    <div style={{...styles.stageOption, ...(timerStage === 'FR' ? styles.stageActive : {})}}>
                        <div style={{fontSize:'12px', fontWeight:'bold', color: timerStage === 'FR' ? '#0F2C59' : '#DAC0A3'}}>STEP 1</div>
                        <div style={{fontWeight:'bold'}}>First Review</div>
                        <div style={{fontSize:'11px', color:'#66785F'}}>0.5x Audio Time</div>
                    </div>
                    <div style={{display:'flex', alignItems:'center', color:'#DAC0A3'}}><ArrowRight size={20}/></div>
                    <div style={{...styles.stageOption, ...(timerStage === 'SV' ? styles.stageActive : {})}}>
                        <div style={{fontSize:'12px', fontWeight:'bold', color: timerStage === 'SV' ? '#0F2C59' : '#DAC0A3'}}>STEP 2</div>
                      ...
                      <div style={{ display: 'flex', gap: '10px' }}>
                        {['Mantis', 'Cricket'].map(type => (
                          <div key={type} onClick={() => (!timerRunning && timerStage === 'FR') && setTimerData({...timerData, client: type})} style={{...styles.radioLabel, ...(timerData.client === type ? styles.radioActive : {}), cursor: (timerRunning || timerStage === 'SV') ? 'not-allowed' : 'pointer', opacity: (timerRunning || timerStage === 'SV') ? 0.7 : 1}}>
                            <div style={{width:'16px', height:'16px', borderRadius:'50%', border:'2px solid', borderColor: timerData.client === type ? '#0F2C59' : '#DAC0A3', display:'flex', alignItems:'center', justifyContent:'center'}}>
                              {timerData.client === type && <div style={{width:'8px', height:'8px', borderRadius:'50%', backgroundColor:'#0F2C59'}} />}
                            </div>
                            {type}
                          </div>
                        ))}
                  ...
                  <div style={{textAlign:'center', marginBottom:'20px'}}>
                    <div style={{fontSize:'14px', color:'#DAC0A3', marginBottom:'4px'}}>
                  ...
                  <div style={{...styles.timerControls, flexDirection: isMobile ? 'column' : 'row'}}>
                    {!timerRunning ? (
                        <button onClick={handleTimerStartPause} style={{...styles.controlBtn, ...styles.startBtn}} disabled={totalTat === 0}>
                            <Play size={16} fill="#F8F0E5" /> Start Timer
                        </button>
                    ) : (
                        <button onClick={handleTimerStartPause} style={{...styles.controlBtn, ...styles.pauseBtn}}>
                            <Pause size={16} fill="#0F2C59" /> Pause
                        </button>
                    )}

                    {timerStage === 'FR' ? (
                        <button onClick={handleFinishFR} style={{...styles.controlBtn, backgroundColor:'#0F2C59', color:'#F8F0E5'}} disabled={totalTat === 0}>
            ...
            {showEntryModal && (
              <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(15, 44, 89, 0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
                <div style={{ backgroundColor: '#F8F0E5', borderRadius: '16px', width: '100%', maxWidth: '480px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
                  <div style={{ padding: '16px 24px', borderBottom: '1px solid #DAC0A3', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 'bold', color: '#0F2C59' }}>{isEditing ? 'Edit Entry' : 'New Entry'}</h2>
                    <button onClick={() => setShowEntryModal(false)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#0F2C59' }}><X size={20}/></button>
                  </div>
                  <form onSubmit={handleSave} style={{ padding: '24px' }}>
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                        <div onClick={() => setFormData({...formData, client: 'Mantis'})} style={{...styles.radioLabel, ...(formData.client === 'Mantis' ? styles.radioActive : {})}}><div style={{width:'16px', height:'16px', borderRadius:'50%', border:'2px solid', borderColor: formData.client === 'Mantis' ? '#0F2C59' : '#DAC0A3', display:'flex', alignItems:'center', justifyContent:'center'}}>{formData.client === 'Mantis' && <div style={{width:'8px', height:'8px', borderRadius:'50%', backgroundColor:'#0F2C59'}} />}</div>Mantis</div>
                        <div onClick={() => setFormData({...formData, client: 'Cricket'})} style={{...styles.radioLabel, ...(formData.client === 'Cricket' ? styles.radioActive : {})}}><div style={{width:'16px', height:'16px', borderRadius:'50%', border:'2px solid', borderColor: formData.client === 'Cricket' ? '#0F2C59' : '#DAC0A3', display:'flex', alignItems:'center', justifyContent:'center'}}>{formData.client === 'Cricket' && <div style={{width:'8px', height:'8px', borderRadius:'50%', backgroundColor:'#0F2C59'}} />}</div>Cricket</div>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}><button type="button" onClick={() => setShowEntryModal(false)} style={{ padding: '10px 16px', border: 'none', background: 'transparent', color: '#0F2C59', fontWeight: '600', cursor: 'pointer' }}>Cancel</button><button type="submit" style={{ padding: '10px 20px', border: 'none', background: '#0F2C59', color: '#F8F0E5', borderRadius: '8px', fontWeight: '600', cursor: 'pointer', boxShadow: '0 4px 6px -1px rgba(15, 44, 89, 0.2)' }}>{loading ? 'Saving...' : 'Save Entry'}</button></div>
                  </form>
                </div>
              </div>
            )}
            ...
            {view === 'list' && (
              <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
                <div style={{ display: 'flex', flexWrap:'wrap', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', gap:'10px' }}>
                  {!isMobile && <div style={{display:'flex', alignItems:'center', gap:'16px'}}><h2 style={{ fontSize: '24px', fontWeight: 'bold', color: '#0F2C59', margin: '0' }}>File History</h2><button onClick={openNewEntry} style={styles.primaryBtn}><Plus size={16} /> Add New</button></div>}
                  <div style={{ display: 'flex', flexDirection:'column', alignItems: 'flex-end', gap: '8px', width: isMobile ? '100%' : 'auto' }}>
                    <div style={{ background: '#EADBC8', color: '#0F2C59', padding: '8px 12px', borderRadius: '8px', fontWeight: '700', fontSize: '13px', border:'1px solid #DAC0A3', display:'flex', gap:'8px', width: isMobile ? '100%' : 'auto', justifyContent: isMobile ? 'center' : 'flex-start' }}><span>Total: {formatDecimalHours(listTotalSeconds)}</span><span style={{opacity:0.6}}>|</span><span>{formatDuration(listTotalSeconds)}</span></div>
                    <div style={{display:'flex', gap:'8px', alignItems:'center', flexWrap: 'wrap', width: isMobile ? '100%' : 'auto'}}>
                        ...
                        <select value={filterType} onChange={(e) => setFilterType(e.target.value)} style={{padding: '8px', borderRadius: '8px', border: '1px solid #DAC0A3', fontSize: '13px', cursor:'pointer', backgroundColor:'white', height:'34px', flex: isMobile ? 1 : 'unset'}}><option value="All">All Types</option><option value="Mantis">Mantis</option><option value="Cricket">Cricket</option></select>
                        <input type="date" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} style={{padding: '8px', borderRadius: '8px', border: '1px solid #DAC0A3', fontSize: '13px', backgroundColor:'white', height:'34px', boxSizing:'border-box', flex: isMobile ? 1 : 'unset'}} />
                  ...
                  <table style={styles.table}>
                    <thead>
                      <tr>
                        ...
                        <th style={{...styles.th, width: `${colWidth}px`, position: 'relative'}}><div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>File Name<GripVertical size={14} style={{color:'#DAC0A3', marginRight:'4px'}} /></div><div onMouseDown={startResizing} style={{position:'absolute', right:0, top:0, bottom:0, width:'8px', cursor:'col-resize', zIndex:10, display:'flex', alignItems:'center', justifyContent:'center'}}><div style={{width:'2px', height:'100%', backgroundColor:'#0F2C59', opacity: 0.3}} /></div></th>
                    ...
                    <tbody>
                      {sortedJobs.length > 0 ? sortedJobs.map(job => (
                        <tr key={job.id} style={{ borderBottom: '1px solid #F8F0E5', backgroundColor: job.file_name.startsWith('Unnamed File') ? '#fee2e2' : 'white' }}>
                          ...
                          <td style={{...styles.td, fontFamily: 'monospace', color:'#0F2C59'}}>{formatDuration(job.total_seconds)}</td>
